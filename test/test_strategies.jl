...
function test_strategies()
    ...
    @testset "OnUpdate Strategy Logic" begin
        ...
        @test RtcFramework.should_publish(strategy, 100, -1, 200, 200) == true   # Property updated now
        @test RtcFramework.should_publish(strategy, 100, -1, 150, 200) == false  # Property not updated now
        @test RtcFramework.should_publish(strategy, -1, -1, 200, 200) == true    # Never published, property updated
        @test RtcFramework.should_publish(strategy, 200, -1, 200, 200) == false  # Already published this update
    ...
    @testset "RateLimited Strategy Logic" begin
        ...
        @test RtcFramework.should_publish(strategy, 100, -1, 200, 200) == false  # Updated now but too soon
        @test RtcFramework.should_publish(strategy, 100, -1, 700, 700) == true   # Updated now and enough time
        @test RtcFramework.should_publish(strategy, -1, -1, 200, 200) == true    # Never published, updated now
