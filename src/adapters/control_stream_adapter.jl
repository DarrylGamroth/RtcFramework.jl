...
"""
    ControlStreamAdapter(subscription::Aeron.Subscription, agent::AbstractRtcAgent)

Create a control stream adapter with the given subscription.
"""
function ControlStreamAdapter(subscription::Aeron.Subscription, agent::AbstractRtcAgent)
    ...
    final_handler = SpidersLateFragmentFilter(filtered_handler, late_fragment_handler,
        props[:LateMessageThresholdNs], base(agent).clock)
